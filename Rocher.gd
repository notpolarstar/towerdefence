...
func _on_Rocher_area_entered(Ennemi):
	if Ennemi.is_in_group("Ennemis") or Ennemi.is_in_group("EnnemisG"):
		if Variables.Volume == true:
			$AudioStreamPlayer.playing = true
		if Ennemi.is_in_group("Ennemis"):
			Variables.Ressources += 10*Variables.Champs
		elif Ennemi.is_in_group("EnnemisG"):
			Variables.Ressources += 20*Variables.Champs
